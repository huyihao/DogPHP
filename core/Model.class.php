...
class Model
{
    ...
    protected static $_link;   // 一个PDO对象
     ...
     * @param $dbName     数据库名
     */
    protected static function connect($host, $username, $password, $dbName){
         try{
             self::$_link = new PDO("mysql:host={$host};dbname={$dbName}",$username,$password);
             //设置返回数据的类型
...
//             self::$_link->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_WARNING);
     ...
     * 查询单条数据的方法
     * @param $table  查询表名
     * @param array $where   查询的条件数据
     * @return mixed   查询返回值数组
     */
